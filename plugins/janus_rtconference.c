/*! \file   janus_rtconference.c
 * \author Ivan Dubynets
 * \copyright GNU General Public License v3
 * \brief  Janus RTConference plugin
 ...
 * A pre-filled configuration file is provided in \c conf/janus.plugin.rtconference.cfg
...
/* Plugin information */
#define JANUS_RTCONFERENCE_VERSION              1
#define JANUS_RTCONFERENCE_VERSION_STRING        "0.0.1"
#define JANUS_RTCONFERENCE_DESCRIPTION           "This is a plugin implementing an audio conference bridge for Janus, mixing Opus streams."
#define JANUS_RTCONFERENCE_NAME                  "JANUS RT Converence plugin"
#define JANUS_RTCONFERENCE_AUTHOR                "Meetecho s.r.l. and Softarex Technologies Inc"
#define JANUS_RTCONFERENCE_PACKAGE               "janus.plugin.rtconference"

/* Plugin methods */
janus_plugin *create(void);
int janus_rtconference_init(janus_callbacks *callback, const char *config_path);
void janus_rtconference_destroy(void);
int janus_rtconference_get_api_compatibility(void);
int janus_rtconference_get_version(void);
const char *janus_rtconference_get_version_string(void);
const char *janus_rtconference_get_description(void);
const char *janus_rtconference_get_name(void);
const char *janus_rtconference_get_author(void);
const char *janus_rtconference_get_package(void);
void janus_rtconference_create_session(janus_plugin_session *handle, int *error);
struct janus_plugin_result *janus_rtconference_handle_message(janus_plugin_session *handle, char *transaction, char *message, char *sdp_type, char *sdp);
void janus_rtconference_setup_media(janus_plugin_session *handle);
void janus_rtconference_incoming_rtp(janus_plugin_session *handle, int video, char *buf, int len);
void janus_rtconference_incoming_rtcp(janus_plugin_session *handle, int video, char *buf, int len);
void janus_rtconference_hangup_media(janus_plugin_session *handle);
void janus_rtconference_destroy_session(janus_plugin_session *handle, int *error);
char *janus_rtconference_query_session(janus_plugin_session *handle);

/* Plugin setup */
static janus_plugin janus_rtconference_plugin =
    JANUS_PLUGIN_INIT(
    .init = janus_rtconference_init,
    .destroy = janus_rtconference_destroy,
    .get_api_compatibility = janus_rtconference_get_api_compatibility,
    .get_version = janus_rtconference_get_version,
    .get_version_string = janus_rtconference_get_version_string,
    .get_description = janus_rtconference_get_description,
    .get_name = janus_rtconference_get_name,
    .get_author = janus_rtconference_get_author,
    .get_package = janus_rtconference_get_package,

    .create_session = janus_rtconference_create_session,
    .handle_message = janus_rtconference_handle_message,
    .setup_media = janus_rtconference_setup_media,
    .incoming_rtp = janus_rtconference_incoming_rtp,
    .incoming_rtcp = janus_rtconference_incoming_rtcp,
    .hangup_media = janus_rtconference_hangup_media,
    .destroy_session = janus_rtconference_destroy_session,
    .query_session = janus_rtconference_query_session,
);

/* Plugin creator */
janus_plugin *create(void) {
    JANUS_LOG(LOG_VERB, "%s created!\n", JANUS_RTCONFERENCE_NAME);
    return &janus_rtconference_plugin;
}
...
static void *janus_rtconference_handler(void *data);
static void janus_rtconference_relay_rtp_packet(gpointer data, gpointer user_data);
static void *janus_rtconference_mixer_thread(void *data);
static void *janus_rtconference_participant_thread(void *data);

typedef struct janus_rtconference_message {
    janus_plugin_session *handle;
    char *transaction;
    json_t *message;
    char *sdp_type;
    char *sdp;
} janus_rtconference_message;
static GAsyncQueue *messages = NULL;

void janus_rtconference_message_free(janus_rtconference_message *msg);
...
typedef struct janus_rtconference_room {
...
} janus_rtconference_room;
static GHashTable *rooms;
static janus_mutex rooms_mutex;

typedef struct janus_rtconference_session {
    janus_plugin_session *handle;
...
} janus_rtconference_session;
static GHashTable *sessions;
static GList *old_sessions;
static janus_mutex sessions_mutex;

typedef struct janus_rtconference_rtp_context {
    /* Needed to fix seq and ts in case of publisher switching */
    uint32_t a_last_ssrc, a_last_ts, a_base_ts, a_base_ts_prev;
    uint16_t a_last_seq, a_base_seq, a_base_seq_prev;
} janus_rtconference_rtp_context;

typedef struct janus_rtconference_participant {
    janus_rtconference_session *session;
    janus_rtconference_room *room; /* Room */
    ...
    janus_rtconference_rtp_context context; /* Needed in case the participant changes room */
    /* Opus stuff */
...
} janus_rtconference_participant;

/* Packets we get from gstreamer and relay */
typedef struct janus_rtconference_rtp_relay_packet {
    rtp_header *data;
    gint length;
    uint32_t ssrc;
    uint32_t timestamp;
    uint16_t seq_number;
} janus_rtconference_rtp_relay_packet;

/* Helper to sort incoming RTP packets by sequence numbers */
static gint janus_rtconference_rtp_sort(gconstpointer a, gconstpointer b) {
    janus_rtconference_rtp_relay_packet *pkt1 = (janus_rtconference_rtp_relay_packet *) a;
    janus_rtconference_rtp_relay_packet *pkt2 = (janus_rtconference_rtp_relay_packet *) b;
...
void *janus_rtconference_watchdog(void *data);
...
void *janus_rtconference_watchdog(void *data) {
    ...
    while (g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
        ...
        if (old_sessions != NULL) {
            ...
            while (sl) {
                janus_rtconference_session *session = (janus_rtconference_session *) sl->data;
                if (!session) {
                    sl = sl->next;
                    continue;
                }
...
int janus_rtconference_init(janus_callbacks *callback, const char *config_path) {
    ...
    /* Read configuration */
    char filename[255];
    g_snprintf(filename, 255, "%s/%s.cfg", config_path, JANUS_RTCONFERENCE_PACKAGE);
    ...
    messages = g_async_queue_new_full((GDestroyNotify) janus_rtconference_message_free);
    ...
    if (config != NULL) {
        ...
        while (cat != NULL) {
            ...
            janus_rtconference_room *rtconference = g_malloc0(sizeof (janus_rtconference_room));
            if (rtconference == NULL) {
                JANUS_LOG(LOG_FATAL, "Memory error!\n");
                cat = cat->next;
                continue;
            }
            rtconference->room_id = atoi(cat->name);
            ...
            rtconference->room_name = description;
            rtconference->is_private = priv && priv->value && janus_is_true(priv->value);
            rtconference->sampling_rate = atoi(sampling->value);
            switch (rtconference->sampling_rate) {
                case 8000:
                case 12000:
                case 16000:
                case 24000:
                case 48000:
                    JANUS_LOG(LOG_VERB, "Sampling rate for mixing: %"SCNu32"\n", rtconference->sampling_rate);
                    break;
                default:
            ...
            if (secret != NULL && secret->value != NULL) {
                rtconference->room_secret = g_strdup(secret->value);
            }
            if (pin != NULL && pin->value != NULL) {
                rtconference->room_pin = g_strdup(pin->value);
            }
            rtconference->record = FALSE;
            if (record && record->value && janus_is_true(record->value))
                rtconference->record = TRUE;
            if (recfile && recfile->value)
                rtconference->record_file = g_strdup(recfile->value);
            rtconference->recording = NULL;
            rtconference->destroy = 0;
            rtconference->participants = g_hash_table_new(NULL, NULL);
            rtconference->destroyed = 0;
            janus_mutex_init(&rtconference->mutex);
            JANUS_LOG(LOG_VERB, "Created rtconference: %"SCNu64" (%s, %s, secret: %s, pin: %s)\n",
                    rtconference->room_id, rtconference->room_name,
                    rtconference->is_private ? "private" : "public",
                    rtconference->room_secret ? rtconference->room_secret : "no secret",
                    rtconference->room_pin ? rtconference->room_pin : "no pin");
            /* We need a thread for the mix */
            GError *error = NULL;
            rtconference->thread = g_thread_try_new("rtconference mixer thread", &janus_rtconference_mixer_thread, rtconference, &error);
            if (error != NULL) {
                /* FIXME We should clear some resources... */
                JANUS_LOG(LOG_ERR, "Got error %d (%s) trying to launch the mixer thread...\n", error->code, error->message ? error->message : "??");
            } else {
                janus_mutex_lock(&rooms_mutex);
                g_hash_table_insert(rooms, GUINT_TO_POINTER(rtconference->room_id), rtconference);
    ...
    while (g_hash_table_iter_next(&iter, NULL, &value)) {
        janus_rtconference_room *ar = value;
    ...
    watchdog = g_thread_try_new("abridge watchdog", &janus_rtconference_watchdog, NULL, &error);
    if (error != NULL) {
    ...
    handler_thread = g_thread_try_new("janus rtconference handler", janus_rtconference_handler, NULL, &error);
    if (error != NULL) {
        g_atomic_int_set(&initialized, 0);
...
int janus_rtconference_get_version(void) {
    return JANUS_RTCONFERENCE_VERSION;
}

const char *janus_rtconference_get_version_string(void) {
    return JANUS_RTCONFERENCE_VERSION_STRING;
}

const char *janus_rtconference_get_description(void) {
    return JANUS_RTCONFERENCE_DESCRIPTION;
}

const char *janus_rtconference_get_name(void) {
    return JANUS_RTCONFERENCE_NAME;
}

const char *janus_rtconference_get_author(void) {
    return JANUS_RTCONFERENCE_AUTHOR;
}

const char *janus_rtconference_get_package(void) {
    return JANUS_RTCONFERENCE_PACKAGE;
}
...
void janus_rtconference_create_session(janus_plugin_session *handle, int *error) {
    ...
    janus_rtconference_session *session = (janus_rtconference_session *) g_malloc0(sizeof (janus_rtconference_session));
    if (session == NULL) {
        JANUS_LOG(LOG_FATAL, "Memory error!\n");
        *error = -2;
        return;
...
void janus_rtconference_destroy_session(janus_plugin_session *handle, int *error) {
    ...
    janus_rtconference_session *session = (janus_rtconference_session *) handle->plugin_handle;
    if (!session) {
    ...
    if (!session->destroyed) {
        ...
        janus_rtconference_hangup_media(handle);
...
char *janus_rtconference_query_session(janus_plugin_session *handle) {
    ...
    janus_rtconference_participant *participant = (janus_rtconference_participant *) session->participant;
    json_object_set_new(info, "state", json_string(participant && participant->room ? "inroom" : "idle"));
    if (participant) {
        janus_rtconference_room *room = participant->room;
...
struct janus_plugin_result *janus_rtconference_handle_message(janus_plugin_session *handle, char *transaction, char *message, char *sdp_type, char *sdp) {
    ...
    if (!strcasecmp(request_text, "create")) {
        /* Create a new rtconference */
        JANUS_LOG(LOG_VERB, "Creating a new rtconference\n");
        ...
        rtconference->room_id = room_id;
        char *description = NULL;
        if (desc != NULL && strlen(json_string_value(desc)) > 0) {
            description = g_strdup(json_string_value(desc));
        } else {
            char roomname[255];
            g_snprintf(roomname, 255, "Room %"SCNu64"", rtconference->room_id);
        ...
        rtconference->room_name = description;
        rtconference->is_private = is_private ? json_is_true(is_private) : FALSE;
        if (secret)
            rtconference->room_secret = g_strdup(json_string_value(secret));
        if (pin)
            rtconference->room_pin = g_strdup(json_string_value(pin));
        if (sampling)
            rtconference->sampling_rate = json_integer_value(sampling);
        else
            rtconference->sampling_rate = 16000;
        switch (rtconference->sampling_rate) {
            case 8000:
            case 12000:
            case 16000:
            ...
            default:
                ...
                JANUS_LOG(LOG_ERR, "Unsupported sampling rate %"SCNu32"...\n", rtconference->sampling_rate);
                error_code = JANUS_AUDIOBRIDGE_ERROR_UNKNOWN_ERROR;
                g_snprintf(error_cause, 512, "We currently only support 16kHz (wideband) as a sampling rate for audio rooms, %"SCNu32" TBD...", rtconference->sampling_rate);
                goto error;
        }
        rtconference->record = FALSE;
        if (record && json_is_true(record))
            rtconference->record = TRUE;
        if (recfile)
            rtconference->record_file = g_strdup(json_string_value(recfile));
        rtconference->recording = NULL;
        rtconference->destroy = 0;
        rtconference->participants = g_hash_table_new(NULL, NULL);
        rtconference->destroyed = 0;
        ...
        if (error != NULL) {
            ...
            g_free(rtconference->room_name);
            g_free(rtconference->room_secret);
            g_free(rtconference->record_file);
            g_hash_table_destroy(rtconference->participants);
            g_free(rtconference);
            goto error;
        } else {
            g_hash_table_insert(rooms, GUINT_TO_POINTER(rtconference->room_id), rtconference);
        ...
        json_object_set_new(response, "rtconference", json_string("created"));
        json_object_set_new(response, "room", json_integer(rtconference->room_id));
        goto plugin_response;
    } else if (!strcasecmp(request_text, "destroy")) {
        JANUS_LOG(LOG_VERB, "Attempt to destroy an existing rtconference room\n");
        json_t *room = json_object_get(root, "room");
        if (!room) {
        ...
        janus_mutex_lock(&rooms_mutex);
        janus_rtconference_room *rtconference = g_hash_table_lookup(rooms, GUINT_TO_POINTER(room_id));
        if (rtconference == NULL) {
            janus_mutex_unlock(&rooms_mutex);
        ...
        if (rtconference->room_secret) {
            ...
            if (!janus_strcmp_const_time(rtconference->room_secret, json_string_value(secret))) {
                janus_mutex_unlock(&rooms_mutex);
        ...
        response = json_object();
        json_object_set_new(response, "rtconference", json_string("destroyed"));
        ...
        GHashTableIter iter;
        gpointer value;
        g_hash_table_iter_init(&iter, rtconference->participants);
        while (g_hash_table_iter_next(&iter, NULL, &value)) {
            janus_rtconference_participant *p = value;
            if (p && p->session) {
                p->room = NULL;
                int ret = gateway->push_event(p->session->handle, &janus_rtconference_plugin, NULL, response_text, NULL, NULL);
                ...
                while (p->inbuf) {
                    ...
                    janus_rtconference_rtp_relay_packet *pkt = (janus_rtconference_rtp_relay_packet *) first->data;
        ...
        rtconference->destroyed = janus_get_monotonic_time();
        g_thread_join(rtconference->thread);
        /* Done */
        ...
        g_hash_table_iter_init(&iter, rooms);
        while (g_hash_table_iter_next(&iter, NULL, &value)) {
            janus_rtconference_room *room = value;
            if (!room)
                continue;
            if (room->is_private) {
                /* Skip private room */
        ...
        janus_mutex_unlock(&rooms_mutex);
        response = json_object();
        json_object_set_new(response, "rtconference", json_string("success"));
        ...
        janus_mutex_lock(&rtconference->mutex);
        json_t *list = json_array();
        GHashTableIter iter;
        gpointer value;
        g_hash_table_iter_init(&iter, rtconference->participants);
        while (!rtconference->destroyed && g_hash_table_iter_next(&iter, NULL, &value)) {
            janus_rtconference_participant *p = value;
        ...
        janus_mutex_unlock(&rtconference->mutex);
        response = json_object();
        json_object_set_new(response, "rtconference", json_string("participants"));
        json_object_set_new(response, "room", json_integer(room_id));
        ...
        janus_rtconference_message *msg = g_malloc0(sizeof (janus_rtconference_message));
        if (msg == NULL) {
            JANUS_LOG(LOG_FATAL, "Memory error!\n");
            error_code = JANUS_AUDIOBRIDGE_ERROR_UNKNOWN_ERROR;
            g_snprintf(error_cause, 512, "Memory error");
...
error:{
        ...
        json_object_set_new(event, "rtconference", json_string("event"));
...
void janus_rtconference_incoming_rtp(janus_plugin_session *handle, int video, char *buf, int len) {
    ...
    if (participant->active && participant->decoder) {
        ...
        janus_rtconference_rtp_relay_packet *pkt = g_malloc0(sizeof (janus_rtconference_rtp_relay_packet));
        if (pkt == NULL) {
            JANUS_LOG(LOG_FATAL, "Memory error!\n");
            return;
        }
        ...
        participant->inbuf = g_list_insert_sorted(participant->inbuf, pkt, &janus_rtconference_rtp_sort);
...
void janus_rtconference_hangup_media(janus_plugin_session *handle) {
    ...
    janus_rtconference_participant *participant = (janus_rtconference_participant *) session->participant;
    janus_rtconference_room *rtconference = participant->room;
    if (rtconference != NULL) {
        janus_mutex_lock(&rtconference->mutex);
        json_t *event = json_object();
        json_object_set_new(event, "rtconference", json_string("event"));
        json_object_set_new(event, "room", json_integer(rtconference->room_id));
        ...
        json_decref(event);
        g_hash_table_remove(rtconference->participants, GUINT_TO_POINTER(participant->user_id));
        GHashTableIter iter;
        gpointer value;
        g_hash_table_iter_init(&iter, rtconference->participants);
        while (g_hash_table_iter_next(&iter, NULL, &value)) {
            ...
            int ret = gateway->push_event(p->session->handle, &janus_rtconference_plugin, NULL, leaving_text, NULL, NULL);
            JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
        }
        g_free(leaving_text);
    }
...
static void *janus_rtconference_handler(void *data) {
    ...
    janus_rtconference_message *msg = NULL;
    int error_code = 0;
    ...
    while (g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
        ...
        janus_rtconference_session *session = (janus_rtconference_session *) msg->handle->plugin_handle;
        if (!session) {
            JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
            janus_rtconference_message_free(msg);
            continue;
        }
        if (session->destroyed) {
            janus_rtconference_message_free(msg);
        ...
        if (!strcasecmp(request_text, "join")) {
            ...
            janus_rtconference_participant *participant = session->participant;
            if (participant != NULL && participant->room != NULL) {
            ...
            if (rtconference->room_pin) {
                ...
                if (!janus_strcmp_const_time(rtconference->room_pin, json_string_value(pin))) {
                    janus_mutex_unlock(&rooms_mutex);
            ...
            if (id) {
                ...
                if (g_hash_table_lookup(rtconference->participants, GUINT_TO_POINTER(user_id)) != NULL) {
            ...
            if (participant == NULL) {
                participant = g_malloc0(sizeof (janus_rtconference_participant));
                if (participant == NULL) {
                    JANUS_LOG(LOG_FATAL, "Memory error!\n");
                    error_code = JANUS_AUDIOBRIDGE_ERROR_UNKNOWN_ERROR;
                    g_snprintf(error_cause, 512, "Memory error");
            ...
            if (participant->encoder == NULL) {
                participant->encoder = opus_encoder_create(rtconference->sampling_rate, 1, OPUS_APPLICATION_VOIP, &error);
                if (error != OPUS_OK) {
                    if (participant->display)
                        g_free(participant->display);
                    g_free(participant);
            ...
            if (participant->decoder == NULL) {
                /* Opus decoder */
                error = 0;
                participant->decoder = opus_decoder_create(rtconference->sampling_rate, 1, &error);
                if (error != OPUS_OK) {
                    if (participant->display)
                        g_free(participant->display);
                    if (participant->encoder)
            ...
            if (participant->thread == NULL) {
                GError *error = NULL;
                participant->thread = g_thread_try_new("rtconference participant thread", &janus_rtconference_participant_thread, participant, &error);
                if (error != NULL) {
            ...
            g_hash_table_insert(rtconference->participants, GUINT_TO_POINTER(user_id), participant);
            /* Notify the other participants */
            json_t *newuser = json_object();
            json_object_set_new(newuser, "rtconference", json_string("joined"));
            json_object_set_new(newuser, "room", json_integer(rtconference->room_id));
            ...
            while (g_hash_table_iter_next(&iter, NULL, &value)) {
                ...
                JANUS_LOG(LOG_VERB, "Notifying participant %"SCNu64" (%s)\n", p->user_id, p->display ? p->display : "??");
                int ret = gateway->push_event(p->session->handle, &janus_rtconference_plugin, NULL, newuser_text, NULL, NULL);
                JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
            }
            g_free(newuser_text);
            ...
            event = json_object();
            json_object_set_new(event, "rtconference", json_string("joined"));
            json_object_set_new(event, "room", json_integer(rtconference->room_id));
            ...
            if (muted) {
                ...
                json_object_set_new(pub, "rtconference", json_string("event"));
                ...
                while (g_hash_table_iter_next(&iter, NULL, &value)) {
                    ...
                    JANUS_LOG(LOG_VERB, "Notifying participant %"SCNu64" (%s)\n", p->user_id, p->display ? p->display : "??");
                    int ret = gateway->push_event(p->session->handle, &janus_rtconference_plugin, NULL, pub_text, NULL, NULL);
                    JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
                }
                g_free(pub_text);
                janus_mutex_unlock(&rtconference->mutex);
            ...
            janus_rtconference_room *old_rtconference = participant->room;
            /* Leave the old room first... */
            janus_mutex_lock(&old_rtconference->mutex);
            g_hash_table_remove(old_rtconference->participants, GUINT_TO_POINTER(participant->user_id));
            janus_mutex_unlock(&old_rtconference->mutex);
            if (old_rtconference->sampling_rate != rtconference->sampling_rate) {
                /* Create a new one that takes into account the sampling rate we want now */
                int error = 0;
                OpusEncoder *new_encoder = opus_encoder_create(rtconference->sampling_rate, 1, OPUS_APPLICATION_VOIP, &error);
                if (error != OPUS_OK) {
                    if (new_encoder)
                    ...
                    janus_mutex_lock(&old_rtconference->mutex);
                    g_hash_table_insert(rtconference->participants, GUINT_TO_POINTER(participant->user_id), participant);
                    janus_mutex_unlock(&old_rtconference->mutex);
                    goto error;
                }
                if (rtconference->sampling_rate == 8000) {
                ...
                /* Opus decoder */
                error = 0;
                OpusDecoder *new_decoder = opus_decoder_create(rtconference->sampling_rate, 1, &error);
                if (error != OPUS_OK) {
                    if (new_encoder)
                        opus_encoder_destroy(new_encoder);
                    new_encoder = NULL;
            ...
            event = json_object();
            json_object_set_new(event, "rtconference", json_string("event"));
            json_object_set_new(event, "room", json_integer(old_rtconference->room_id));
            json_object_set_new(event, "leaving", json_integer(participant->user_id));
            char *leaving_text = json_dumps(event, JSON_INDENT(3) | JSON_PRESERVE_ORDER);
            GHashTableIter iter;
            gpointer value;
            g_hash_table_iter_init(&iter, old_rtconference->participants);
            while (g_hash_table_iter_next(&iter, NULL, &value)) {
                janus_rtconference_participant *p = value;
                if (p == participant) {
                    continue; /* Skip the new participant itself */
            ...
            event = json_object();
            json_object_set_new(event, "rtconference", json_string("roomchanged"));
            json_object_set_new(event, "room", json_integer(rtconference->room_id));
            json_object_set_new(event, "id", json_integer(user_id));
            json_object_set_new(event, "participants", list);
            janus_mutex_unlock(&rtconference->mutex);
        ...
        /* Any SDP to handle? */
        if (!msg->sdp) {
            int ret = gateway->push_event(msg->handle, &janus_rtconference_plugin, msg->transaction, event_text, NULL, NULL);
            JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
        } else {
            ...
            int res = gateway->push_event(msg->handle, &janus_rtconference_plugin, msg->transaction, event_text, type, sdp);
...
static void *janus_rtconference_mixer_thread(void *data) {
    ...
    janus_rtconference_room *rtconference = (janus_rtconference_room *) data;
    if (!rtconference) {
        JANUS_LOG(LOG_ERR, "Invalid room!\n");
        return NULL;
    }
    JANUS_LOG(LOG_VERB, "Thread is for mixing room %"SCNu64" (%s) at rate %"SCNu32"...\n", rtconference->room_id, rtconference->room_name, rtconference->sampling_rate);

    /* Do we need to record the mix? */
    if (rtconference->record) {
        char filename[255];
        if (rtconference->record_file)
            g_snprintf(filename, 255, "%s", rtconference->record_file);
        else
            g_snprintf(filename, 255, "/tmp/janus-audioroom-%"SCNu64".wav", rtconference->room_id);
        rtconference->recording = fopen(filename, "wb");
        if (rtconference->recording == NULL) {
            ...
            if (fwrite(&header, 1, sizeof (header), rtconference->recording) != sizeof (header)) {
    ...
    int samples = rtconference->sampling_rate / 50;
    ...
    while (!g_atomic_int_get(&stopping) && rtconference->destroyed == 0) { /* FIXME We need a per-room watchdog as well */
        ...
        janus_mutex_lock_nodebug(&rtconference->mutex);
        count = g_hash_table_size(rtconference->participants);
        janus_mutex_unlock_nodebug(&rtconference->mutex);
        if (count == 0) {
            /* No participant, do nothing */
            if (prev_count > 0) {
                JANUS_LOG(LOG_VERB, "Last user just left room %"SCNu64", going idle...\n", rtconference->room_id);
                prev_count = 0;
            }
            continue;
        }
        ...
        janus_mutex_lock_nodebug(&rtconference->mutex);
        GList *participants_list = g_hash_table_get_values(rtconference->participants);
        janus_mutex_unlock_nodebug(&rtconference->mutex);
        for (i = 0; i < samples; i++)
            buffer[i] = 0;
        GList *ps = participants_list;
        while (ps) {
            janus_rtconference_participant *p = (janus_rtconference_participant *) ps->data;
            janus_mutex_lock(&p->qmutex);
            ...
            janus_rtconference_rtp_relay_packet *pkt = (janus_rtconference_rtp_relay_packet *) (peek ? peek->data : NULL);
            if (pkt != NULL) {
        ...
        if (rtconference->recording != NULL && g_list_length(participants_list) > 0) {
            for (i = 0; i < samples; i++) {
                /* FIXME Smoothen/Normalize instead of truncating? */
                outBuffer[i] = buffer[i];
            }
            fwrite(outBuffer, sizeof (opus_int16), samples, rtconference->recording);
        }
        /* Send proper packet to each participant (remove own contribution) */
        ps = participants_list;
        while (ps) {
            janus_rtconference_participant *p = (janus_rtconference_participant *) ps->data;
            janus_rtconference_rtp_relay_packet *pkt = NULL;
            janus_mutex_lock(&p->qmutex);
            if (p->active && !p->muted && !p->prebuffering && p->inbuf) {
                GList *first = g_list_first(p->inbuf);
                pkt = (janus_rtconference_rtp_relay_packet *) (first ? first->data : NULL);
            ...
            janus_rtconference_rtp_relay_packet *mixedpkt = g_malloc0(sizeof (janus_rtconference_rtp_relay_packet));
            if (mixedpkt != NULL) {
                ...
                if (mixedpkt->data == NULL) {
                    ...
                    mixedpkt->ssrc = rtconference->room_id;
    ...
    if (rtconference->recording)
        fclose(rtconference->recording);
    JANUS_LOG(LOG_VERB, "Leaving mixer thread for room %"SCNu64" (%s)...\n", rtconference->room_id, rtconference->room_name);

    /* Free resources */
    g_free(rtconference->room_name);
    g_free(rtconference->room_secret);
    g_free(rtconference->room_pin);
    g_free(rtconference->record_file);
    g_hash_table_destroy(rtconference->participants);
    g_free(rtconference);
...
static void *janus_rtconference_participant_thread(void *data) {
    ...
    janus_rtconference_participant *participant = (janus_rtconference_participant *) data;
    if (!participant) {
    ...
    janus_rtconference_session *session = participant->session;

    /* Output buffer */
    janus_rtconference_rtp_relay_packet *outpkt = g_malloc0(sizeof (janus_rtconference_rtp_relay_packet));
    if (outpkt == NULL) {
        JANUS_LOG(LOG_FATAL, "Memory error!\n");
        g_thread_unref(g_thread_self());
        return NULL;
    ...
    janus_rtconference_rtp_relay_packet *mixedpkt = NULL;
        ...
        if (mixedpkt != NULL && session->destroyed == 0) {
            ...
            if (participant->active && participant->encoder) {
                ...
                if (outpkt->length < 0) {
                    ...
                    /* Backup the actual timestamp and sequence number set by the rtconference, in case a room is changed */
                    outpkt->ssrc = mixedpkt->ssrc;
                    outpkt->timestamp = mixedpkt->timestamp;
                    outpkt->seq_number = mixedpkt->seq_number;
                    janus_rtconference_relay_rtp_packet(participant->session, outpkt);
                }
            }
            if (mixedpkt) {
                if (mixedpkt->data)
    ...
    while (g_async_queue_length(participant->outbuf) > 0) {
        janus_rtconference_rtp_relay_packet *pkt = g_async_queue_pop(participant->outbuf);
        if (pkt == NULL)
            continue;
        if (pkt->data)
            g_free(pkt->data);
...
static void janus_rtconference_relay_rtp_packet(gpointer data, gpointer user_data) {
    janus_rtconference_rtp_relay_packet *packet = (janus_rtconference_rtp_relay_packet *) user_data;
    if (!packet || !packet->data || packet->length < 1) {
        JANUS_LOG(LOG_ERR, "Invalid packet...\n");
        return;
    }
    janus_rtconference_session *session = (janus_rtconference_session *) data;
